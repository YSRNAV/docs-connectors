...
The following use case contains flows that send and receive messages for topics and queues:

* load-http-form-flow
+
Provides a web form for entering the parameters. The form is used form to send the message content to the queue or topics.
+
* topic-endpoint-flow
+
Receives a message through the `load-http-form-flow`, stores the message as the payload, converts it to a JAVA object, and sends it to the destination topic.
+
* queue-endpoint-flow
+
Receives a message through the `load-http-form-flow`, stores the message as the payload, converts it to a JAVA object and sends it to the destination queue.
+
* queue-receive-flow
+
Listens for, receives, and logs messages from a specified queue.
+
* topic-receive-flow
+
Listens for, receives, and logs messages from a specified topic.
...
The following use case contains flows that create topics, subscriptions, and rules:

*service-bus-management-demoFlow
+
Creates topics as follows:
...
+
* Creates subscriptions in a specified topic by following these steps:
. Creates the subscription description with the desired configuration and stores it as the payload.
...
+
. Creates the rule with the desired configuration and stores it as the payload.
